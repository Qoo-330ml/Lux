...
CREATE TABLE recommendation_item_stats (
    ...
    recent_playback_score INTEGER NOT NULL DEFAULT 0 CHECK (recent_playback_score >= 0 AND recent_playback_score <= 50),
    favorite_score INTEGER NOT NULL DEFAULT 0 CHECK (favorite_score >= 0 AND favorite_score <= 50),
    refreshed_batch_key INTEGER NOT NULL
);

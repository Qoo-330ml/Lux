...
服务端转码、Remux 或代理媒体内容。对于包含 HEVC 视频和 AAC-LC/AC-3 音频的 MKV，需要一个不改变视频编码的
浏览器端路径。

## 决定

- 在 Web Worker 内流式解封装 MKV，将 HEVC 样本转换为 fMP4 所需的 length-prefixed 形式，并将 AAC-LC 或
  AC-3 样本写入同一组 A/V fMP4 片段；AC-3 的 `dac3` 配置从首个音频帧解析。
- HEVC 配置以原始 `hvcC` Box 写入 `hvc1` 样本描述，避免 MP4Box 在浏览器构建中再次解析 Matroska
  `CodecPrivate`。
- 浏览器同时支持组合 HEVC+AC-3 MSE 时才为 AC-3 选择该路径；不支持时继续使用已有的客户端 HEVC 解码、
...
- MKV 仍只承诺 HEVC + AAC-LC/AC-3；DTS、Opus、PGS/ASS 和复杂字幕轨需要其他路径或兼容客户端。

...
- [x] 媒体库创建、根路径添加、实时监听、全量校验启停和手动扫描入口连接真实 API。
...
- [x] 任务与日志页按注册表集中查看注册任务、运行记录和脱敏日志；任务只能由系统或插件注册，媒体库创建时自动注册全量校验媒体库和元数据刮削两项任务，实时增量扫描由文件系统事件触发，页面仅维护已注册项的启停，执行时间由宿主机 crontab 管理。

...
> LUX-158 在不破坏本任务 URL 兼容行为的前提下，将 `.strm` 的原始内容扩展为可分类的播放目标；路径型目标的播放策略不在 LUX-072 内完成。

## 范围

扫描 `.strm` 文件并把首个非空文本行作为原始播放目标，交给有媒体库权限的客户端或后续目标处理策略。Lux 不验证 URL、不发起 HEAD/GET、不调用外部服务，也不通过自身代理数据；扫描阶段也不读取外部源的容器信息、索引或媒体轨。

## 实现

- [x] 电影、剧集和混合库扫描器识别 `.strm`。
- [x] 读取首个非空行，清理 BOM 和首尾空白；空文件安全保存为无地址 source。
- [x] 保存 URL 型 `.strm` 的 `STRM_URL` source kind 和明文 `external_url`，重扫时更新目标。
- [x] PlaybackInfo 对 URL 型目标返回外部 URL、Http 协议、远程标记和 DirectPlay 能力，不执行网络探测。

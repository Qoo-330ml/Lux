...
| Hills | 1.8.0 | Android 17，23127PN0CC，arm64-v8a；宿主 macOS arm64 | 未单独验证（已有连接） | 未单独验证（已有会话） | 通过 | 通过；NextEmby `.strm` 代理场景需使用本次 Hills token URL 兼容修复 | 未单独验证 | 未测试 | 2026-08-28 使用独立资源《7天》进入 Hills MPV 播放器并稳定出画面；2026-09-05 HAR 复现 Hills 丢失独立媒体请求鉴权导致 NextEmby `User: unknown`，Lux 已在 Hills 签名播放 URL 写入 `api_key`；需部署后复测 |
...
- 2026-08-28 通用 Emby 无头播放兼容：发现 HILLS 等 Android 播放器也可能在独立媒体请求中丢失 Emby 鉴权头。`PlaybackInfo` 现在对所有可播放媒体源生成绑定用户、条目和媒体源的短期 HMAC `DirectStreamUrl`，普通客户端的 `AddApiKeyToDirectStreamUrl` 为 `false` 且不把长期 token 写入 URL；标准 `X-Emby-Token`、`X-Emby-Authorization` 和 `api_key` 仍继续接受。`.strm` 播放入口继续使用媒体请求中的播放器 User-Agent 解析有限重定向；自动化回归已覆盖无头本地播放、远程 307 和协议解析器路径。HILLS/Yamby 真机需在远端部署后重新复测。
...
- 2026-09-05 Hills/NextEmby 播放身份兼容修复：Hills 1.8.0 的独立媒体请求只携带 Lux 短期 `luxPlayback...` 票据，且实测忽略 `AddApiKeyToDirectStreamUrl`，NextEmby 无法用自定义参数识别播放用户，因而在回源前记录 `User: unknown` 并拦截。Lux 现在在同一签名 URL 中补充标准 `UserId` 身份关联提示，并在 `X-Emby-Client=Hills` 的 PlaybackInfo 中将 `AddApiKeyToDirectStreamUrl` 设为 `true`，同时把本次标准 Emby token 作为 `api_key` 写入 Hills 专用签名 URL 供 NextEmby 识别；真正授权仍由 Lux 绑定条目、媒体源和用户的 HMAC 票据完成，不开放匿名媒体访问。普通客户端仍不携带长期 token。远端部署后需用 Hills 1.8.0 重新验证 `stream.matroska,webm` 与 `stream.mov,mp4,m4a,3gp,3g2,mj2` 两种请求。

...
- 2026-09-05 Hills/NextEmby 播放身份兼容修复：Hills 的独立媒体请求只携带 Lux 短期 `luxPlayback...` 票据，NextEmby 无法用该自定义参数识别播放用户，因而在回源前记录 `User: unknown` 并拦截。Lux 现在在同一签名 URL 中补充标准 `UserId` 身份关联提示，并在 `X-Emby-Client=Hills` 的 PlaybackInfo 中将 `AddApiKeyToDirectStreamUrl` 设为 `true`，让 Hills 按 Emby 规范追加当前 token 供 NextEmby 识别；真正授权仍由 Lux 绑定条目、媒体源和用户的 HMAC 票据完成，不开放匿名媒体访问。远端部署后需用 Hills 1.8.0 重新验证 `stream.matroska,webm` 与 `stream.mov,mp4,m4a,3gp,3g2,mj2` 两种请求。

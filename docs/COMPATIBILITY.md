...
- Emby `GET /Persons/{personIdOrName}` 已与人物列表 DTO 对齐，优先按人物 ID、未匹配时按精确姓名查询，并支持 `/emby` 前缀、`Fields`、共享 API Key、用户媒体库 ACL 和人物图片标签；为兼容 MDC，`GET/HEAD /Items/{personId}` 也返回相同人物 DTO，已鉴权的无 tag `Primary` 人物图片请求同样受支持。Lux Web 提供受 session/API Key 保护的 `GET /api/v1/people/{personId}` 人物详情，供人物详情页读取。人物详情不因缺少简介或头像而返回空响应；尚未以目标第三方客户端真实演员刮削流程复测。

...
- 2026-08-30 Lux Web 远程 HTTP(S) `.strm` 直连回退：为验证部分 302 服务依赖完整浏览器请求上下文的资源，Web 播放器对 URL 型 `.strm` 暂时直接使用 `.strm` 原始地址，并跳过客户端 MKV/HEVC 脚本读取 fallback；浏览器自行跟随 302 并承担媒体流量。Emby/第三方客户端仍使用 Lux 播放入口、播放器 User-Agent 转发和有限 307 重定向。该 Web 行为要求上游允许浏览器直接访问，复杂格式兼容性不在此次变更范围内。

...
- 2026-08-30 Lux Web 远程 HTTP(S) `.strm` 307 验证回退：Web 播放器恢复先请求 Lux 的短期签名播放入口，由 Lux 使用入站播放器 User-Agent 解析上游有限重定向并返回 307；媒体字节仍由浏览器直连最终地址。该记录用于验证依赖请求上下文的 302 服务，复杂格式兼容性仍按浏览器和现有客户端 fallback 能力单独判断。

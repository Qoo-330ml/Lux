...
不因该参数创建转码会话。HLS 清单通过标准 `master.m3u8` 入口返回，fMP4 的 init/m4s 或 MPEG-TS 的 ts 片段使用同一转码会话的
签名 URL；转码 URL 同时带有 Emby 客户端通常依赖的 `DeviceId`、输出 codec、码率、轨道索引和会话容器分片参数，实际
...
`MediaSources[]` 都返回完整的 `RunTimeTicks`，优先使用选中 source 时长并回退到条目时长；Emby HLS 清单按完整时长立即返回从 `MEDIA-SEQUENCE:0` 开始的 VOD 时间轴、完整分片列表和 `ENDLIST`。`PlaybackInfo` 和已知总时长的 master 清单只登记并保留会话，不占用 FFmpeg 并发名额，也不替换同一 source 正在播放的旧会话；首个 init 或媒体分片请求才替换旧会话、取得名额并启动 generation 0，首个媒体分片可直接决定起点，避免先启动第 `0` 段再被 Harbor 的中段请求立即取消。总时长未知而无法合成完整清单时，物理 `index.m3u8` 请求本身作为首个资源启动 FFmpeg。若 init 先到，合法范围内的 `StartTimeTicks` 只作为首选起点；已知总时长下等于或超过总时长的提示会被忽略。Emby HLS 不把中段物理分片映射为逻辑第 `0` 段；缺失且远离当前输出窗口的分片会按逻辑编号对应的媒体位置重启 FFmpeg。视频转码档位使用四秒关键帧节奏，复制视频流时实际边界仍可落在源关键帧；`-copyts -avoid_negative_ts disabled` 的合同是保留源时间线并避免每次重启从零计时，不承诺任意源文件的绝对 PTS 必然等于 `分片号 × 4 秒`。后续重启使用独立的内部 generation 清单和 init 文件；init 与媒体分片并发请求会跟随当前 generation，旧的并发媒体分片请求不会反向覆盖更新的 seek。重启前重新检查磁盘水位和会话配额，重启失败会终止会话并释放转码名额。客户端省略 `StartTimeTicks` 时，Lux 仍会把未标记为已播放的保存进度作为转码 URL 兼容提示，客户端显式发送的有效值（包括 `0`）优先；实际媒体内容由客户端请求的逻辑分片决定。Lux 自有签名参数仍保留，长期 API token 不写入转码 URL。

`tests/playback.rs` 已覆盖 Direct Play 优先、本地转码 PlaybackInfo 协商、保存进度对应的第 `1230` 段作为首个请求、随后回到第 `0` 段，以及 manifest/init/segment 实际读取；播放模块测试另覆盖登记会话后不预启动或占用 FFmpeg、未消费的新 offer 不停止旧播放、首个中段请求直接启动 generation 0、init 与 seek 分片并发时跟随当前 generation、越界起点拒绝、真实后向 generation 重启、并发 seek 取代、重启配额和失败清理，
跨条目和篡改签名拒绝、播放回调刷新/停止清理，以及 URL/路径型 `.strm` 对外声明转码能力但不创建 HLS 会话。该自动化证据
...
仍依赖客户端补发 `PlaySessionId` 或活动会话自然过期。该自动化回归不替代 FNOS 部署后真实播放、停止回调和 nextEmby 卡片生命周期复测。
...
资源签名都绑定同一个会话容器，TS 与 fMP4 不可混用。服务端回归已覆盖无容器默认 TS、显式 mp4 fMP4、
中段启动/回到开头、分片 Content-Type 和容器篡改拒绝；这仍不替代 FNOS 部署后的 Harbor 真机首帧、seek、
音画和进度验证。
...
Migration 0135 新增 discovery format 3 的紧凑存在性 ledger；后续 additive migration 为 v3 正向索引记录 `last_seen_change_kind` 并增加可恢复的 postprocessing-target cursor/readiness。Format 1/2 及其活动执行器保持不变，现存任务按 ready 默认值跳过新 target 阶段；新任务显式启用 v3 target checkpoint。成功正向索引和稳定 unchanged path 都由 `filesystem_entries.last_seen_generation` 表示；v3 ledger 只保存 `(manifest_id, library_root_id, relative_path)` 且只用于未能安全推进 generation 的观察路径，例如准备不稳定或 CAS 冲突。root/目录身份 observation 仍保留在 `scan_manifest_entries`，ledger、generation 标记、正向索引、目录 frontier 和进度原子提交。SOURCE/ITEM targets 在索引完成事件与首页切换后，按每根路径游标分批物化；全局 ready barrier 与最后一个 root 的完成状态原子提交，所有 metadata/probe/thumbnail worker 等待该 barrier。重试保持已有 target 状态且从已提交游标继续。
...
- 2026-09-19 Symedia 媒体库发现兼容：Emby 兼容层新增管理员 `GET /Library/SelectableMediaFolders`（支持 `/emby` 前缀），按真实 Emby 的顶层数组结构返回媒体库及其 `SubFolders` 根目录，避免 Symedia 将 404 错误对象当作目录数组解析。自动化覆盖共享 API Key、两种路由形式以及媒体库和根目录字段；尚未在真实 Symedia 部署中复测。
...
- 2026-09-23 Hills 1.8.0 Android 集详情媒体信息抓包：同一分集的 `GET /Users/{userId}/Items/{itemId}?Fields=ProviderIds,ExternalUrls,Trickplay` 和 `POST /Items/{itemId}/PlaybackInfo` 均返回 200。Emby 的详情响应即使 `Fields` 未请求，也包含 `MediaSources`、顶层 `RunTimeTicks` 和 `Container`；Lux 详情响应含顶层 `MediaStreams`，但缺少 `MediaSources`、`RunTimeTicks`、`Container`、`Size` 和 `Bitrate`。Lux 的 `PlaybackInfo` 已返回媒体源和流信息，但源 `Name` 是质量标签，Emby 使用文件名。Lux 现已在 Emby 单条详情 DTO 中补齐媒体源、源内媒体流及上述媒体摘要字段，目录列表仍尊重 `Fields`。这与 Hills 详情页媒体信息区为空的表现吻合；补丁尚未部署，页面显示效果尚未复验。Lux 当前目录投影没有逐媒体源的原始文件名，故 `PlaybackInfo.Name` 差异仍待有可靠文件名来源后处理。
...
- 2026-09-23 LUX-234 Emby 条目路径兼容修正：URL/路径型 `.strm` 的 Emby item DTO `Path` 现在返回已索引的 `.strm` 文件实际路径，以匹配旧 Emby；`MediaSources[].Path` 仍是 STRM 原始目标，其他播放字段不变。路径从授权条目选中媒体源关联的库根目录和相对路径构造，并拒绝绝对路径或含 `..` 的相对路径。`tests/strm.rs` 覆盖详情和列表 DTO；NextEmby/FNOS 的实际播放结果仍需部署后复测。
- 2026-09-23 NextEmby 播放交接复核：旧 Emby 抓包的 URL 型 `.strm` 流交接使用 302，并在直放 URL 中携带 `DeviceId`；Lux 原先返回 307 且漏带已从 PlaybackInfo 请求识别的 `DeviceId`。Lux 的 Emby 标准流入口现返回 302，签名 `DirectStreamUrl` 携带已知设备 ID；Lux 自有播放回退仍为 307。Reqable/FNOS 记录中的首次 403 后续有 302 与媒体 206，但 403 的具体发出层尚未由服务端日志确认，因此需部署后再次用 NextEmby 验证首帧与连续播放；自动化回归覆盖 302 和 DeviceId。

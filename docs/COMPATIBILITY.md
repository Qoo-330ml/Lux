...
- 2026-08-28 混合媒体库兼容修复：普通 Emby 客户端继续在混合库 DTO 中收到 `CollectionType: "mixed"`；VidHub、Yamby 等使用旧混合库形状的认证设备收到 `CollectionType: null`，同时保留电影/剧集条目列表和 `TypeOptions`。服务端通过集中兼容名单复用这条策略，后续发现同类客户端时只需加入名单；`tests/mixed_library_api.rs` 覆盖普通 Emby 与 VidHub/Yamby 的分流。

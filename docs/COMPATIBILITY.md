...
- Emby `GET /Persons/{personId}` 已与人物列表 DTO 对齐，并支持 `/emby` 前缀、`Fields`、共享 API Key、用户媒体库 ACL 和人物图片标签；Lux Web 提供受 session/API Key 保护的 `GET /api/v1/people/{personId}` 人物详情，供人物详情页读取。人物详情不因缺少简介或头像而返回空响应；尚未以目标第三方客户端真实演员刮削流程复测。

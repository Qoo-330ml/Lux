# LUX-183：通知器插件与 Webhook 实施计划

## 当前范围

LUX-183 建立统一的通知事件、持久化 outbox 和投递记录；实际渠道由独立的通知器插件负责。
Webhook 是首个兼容 provider：旧版 Webhook 目标继续可用，新目标通过 `notification.send` 插件协议投递。
播放事件使用独立节流规则，Lux 原生和 Emby 风格 payload 使用独立 DTO adapter，不污染 Lux 原生合同。
Telegram、企业微信和 Email 的具体插件包不属于本次交付。
...
- 通知器 adapter 只接收已脱敏的统一事件；secret 受限存储，通过受控 RPC 传给 provider，插件不能读取整个
  `/config`。HTTP 超时、重试和错误分类由宿主统一负责。
- provider 通过 `notification` manifest 类型和 `notification.send` RPC 接入；宿主只调度已安装、已启用且能力声明
  正确的插件。旧 Webhook 行保留 `builtin.webhook` 兼容 provider，迁移不破坏已有目标。
...
- 增加 `notification_destinations`、`notification_events`、`notification_deliveries`；目标记录 provider 插件 ID
  和非秘密配置，旧 Webhook 字段保留用于兼容迁移。
- 添加空库 migration、幂等约束和分页查询。

### 2. 通知器插件协议

- manifest 支持 `type: "notification"`、`category: "NOTIFICATION"` 和 `capabilities: ["notification.send"]`。
- `notification.send` 接收统一事件、目标非秘密配置和受控 secret；返回 `DELIVERED`、`RETRYABLE` 或 `FAILED`。
- 通知插件进程不继承完整 `LUX_CONFIG_DIR`，不得从文件系统读取其他插件或系统 Secret。
...
当前交付不包含 Telegram、企业微信、Email 或 SMTP 的具体插件实现；这些插件只需遵循已定义的 provider 协议，
后续可独立发布，不改动 Lux 通知队列和事件合同。

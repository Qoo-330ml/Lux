...
同步 `test` 到 `main` 时，在 GitHub Actions 手动运行 `.github/workflows/sync-test-to-main.yml`。该 workflow 需先进入默认分支才可运行；运行后会创建或更新 `test` → `main` Pull Request，审核并合并后由 `main` 的发布 workflow 自动更新 `latest`。

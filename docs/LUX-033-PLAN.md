...
- [x] 扫描 worker 完成库扫描后探测仍为 `PENDING` 的本地媒体源；`.strm` 仅处理可用旁车，显式 STRM 探测任务才通过插件读取支持的目标。
...
- 不在 Lux 核心对 `.strm` 指向的外部媒体运行 ffprobe；显式探测由受监督插件完成，首次播放按目标类型由客户端重定向、Lux 本地读取或解析器处理。

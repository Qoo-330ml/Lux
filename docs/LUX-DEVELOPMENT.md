...
- 每个媒体库默认实时监听文件系统；新增、修改、重命名和删除事件只触发受影响路径的局部增量扫描。实时监听不是可关闭的媒体库开关；旧版 `realtime_watch_enabled` 字段仅作兼容保留并始终按开启处理。媒体库另有独立的 `realtime_metadata_auto_match_enabled` 开关，默认关闭；开启后，局部增量扫描完成并确认有可用媒体条目时，按 `FILL_MISSING` 提交受影响条目的在线元数据补全任务。全量校验和元数据任务可分别由宿主机 crontab 配置；Lux 只负责校验注册项并把请求加入持久化后台队列。局部增量扫描由实时事件触发，不作为管理员可配置的计划任务。
...
- plugin_id（可空）
- cron_or_interval（历史兼容字段；宿主机 crontab 是计划来源，Lux 不解析或持久化 cron 表达式）
- is_enabled
- resource_limit_json
...
实时增量扫描默认只更新索引；媒体库开启 `realtime_metadata_auto_match_enabled` 后，增量任务完成时只为本次受影响且仍可用的媒体条目提交 `FILL_MISSING` 元数据任务，不对整库重新刮削。NFO、图片和其他旁车文件的写回事件不得直接导致同一条目无限重复提交；已完整补全的条目由元数据任务跳过。
...
- 扫描计划与元数据计划，明确分开。
- 扫描/任务页。
- 任务与日志页集中查看所有已注册的任务、运行记录和脱敏日志。任务不能由 Web 管理员凭空创建，只能由 Lux 系统或插件注册；管理员只能维护已注册任务的启停和资源配置。具体执行时间由宿主机 crontab 管理。
- 空库初始没有注册任务。创建媒体库时由系统原子注册“全量校验媒体库”和“元数据刮削”两个任务；任务默认停用，管理员启用后才能接受宿主机 crontab 的入队请求。实时增量扫描由文件系统监听触发，不注册为计划任务。
- 待处理匹配页。
- 元数据编辑与锁定。
- 图片管理。
- 用户与权限。
...
- WRITE_NFO
- DOWNLOAD_IMAGE
- AUTO_LIBRARY_COVER（每个媒体库首次达到海报阈值时注册并自动执行一次，不接受计划调度，但允许管理员手动执行）
- MATCH_DANMAKU
- WRITE_DANMAKU_XML
- REBUILD_SEARCH
- PURGE_MISSING
...
Lux 不运行内部计划轮询器，也不解析 cron 表达式。管理员在宿主机 crontab 中配置时间，crontab
通过受保护的接口请求 Lux 入队：

```text
POST /api/v1/cron/tasks/{ownerType}/{ownerId}/{taskType}
Authorization: Bearer <LUX_CRON_TOKEN>
```

允许的计划任务为 `LIBRARY/RECONCILIATION_SCAN`、`LIBRARY/METADATA_PARSE` 和
`GLOBAL/global/STRM_MEDIA_INFO`。接口只校验任务注册项、启用状态和资源边界，然后返回 `202` 并
启动已有的持久化后台任务；扫描、刮削、网络访问和 `ffprobe` 不在接口请求中执行。任务已在运行时返回
冲突，crontab 可以忽略该次重复入队。

令牌通过 `LUX_CRON_TOKEN` 或配置目录中的 `cron-token` 文件注入，服务启动时读取；令牌不得写入
日志、数据库、任务事件或 API 响应。宿主机 crontab 是计划的唯一来源，Lux 管理页不编辑宿主机
crontab。
...
- 文件扫描与元数据任务统一在任务与日志页查看和启停；具体时间由宿主机 crontab 配置，媒体库编辑页不提供计划字段。
- 显示读写与监听状态。
...
- 创建媒体库后自动出现两个系统注册任务：全量校验媒体库、元数据刮削；注册项包含稳定类型、名称、说明、作用范围和注册来源。任务页显示宿主机 crontab 配置提示；实时增量扫描由文件系统监听触发，不出现在计划任务列表中。
- 查看、取消、重试运行中的任务。
...
插件启用后，宿主自动登记一个全局 `STRM_MEDIA_INFO` 计划任务；任务读取同一份插件配置。插件禁用
时任务保留但停用；宿主机 crontab 通过 cron 入队接口执行。未完成有效配置时只登记未启用的任务，
不创建探测作业。
...
- [ ] 插件详情页展示并保存媒体库多选、并发数、已有媒体信息处理方式和旁车写回配置；计划时间由宿主机 crontab 管理，配置文件原子保存且权限受限，插件列表回显非敏感值。
...
- 不把计划任务执行放入普通扫描、播放或用户请求路径；cron 入队接口只复用已有 STRM 后台作业服务。

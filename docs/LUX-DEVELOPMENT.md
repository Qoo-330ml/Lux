...
- ffprobe：仅用于新增或变化媒体的技术信息、时长、内嵌轨道和章节探测。
...
- 提交密码、TMDb token、真实 .strm URL 或用户数据。
...
- 管理员在初始化或设置页提供 API Read Access Token。
- token 只保存在 /config 中的受限配置或 secrets 文件，不返回普通用户。
...
- 不执行 URL 合法性检查或 HEAD 请求。
- PlaybackInfo 将其作为直接媒体源返回。
- 不通过 Lux 转发数据。
- 若目标客户端拒绝外部 URL，则记录为客户端兼容性问题，再根据真实请求决定是否增加可选代理；不得默认扩大首版。

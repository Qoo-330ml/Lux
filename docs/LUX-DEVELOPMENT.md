...
- ffprobe 默认并发 32，可按媒体库配置 1 至 64；实际运行的单库有效上限为 64、进程全局硬上限为 64，
  并根据 CPU、内存和前台 p95 动态降档。4 核 NAS 的默认有效并发目标为 32；ffprobe 只处理本轮
...
ffprobe 使用独立的有界资源配额：默认 32 路，单库有效上限 64，进程全局硬上限 64；配置值
保留 1 至 64 的输入范围，但实际并发受 CPU、内存、前台 p95 和全局 semaphore 限制。后处理
不持有文件扫描互斥锁，不能把无限数量的 ffprobe、NFO 或图片任务一次性提交到 Tokio worker。

验收：

- [ ] 无变化全量重扫只执行目录读取、stat/fingerprint 和批量 seen 更新；不会调用媒体索引、NFO、图片、缩略图或 ffprobe。
- [ ] 单个新增、变化、删除和旁车变化只派生对应 source/item target；同一 item 的多个 source 不重复处理 item 级任务。
- [ ] 扫描目标、后处理阶段和失败状态持久化；进程重启、取消和重试不会重复完成目标，也不会丢失未完成目标。
- [ ] ffprobe 默认有效并发为 32，硬上限为 64；CPU、内存或前台 p95 恶化时能动态降档，恢复后带冷却地升档。
- [ ] reconciliation 工作项的发现、seen、变化目标登记和完成清理使用有界批量事务，SQLite 不逐文件往返。
- [ ] 16/32/48/64 路 ffprobe 和 60,000 文件首扫/无变化重扫均有可重复基准记录；扫描期间前台 p95 保持小于 1 秒或记录差距。

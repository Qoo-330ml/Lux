...
- [ ] 新人物布局允许一个规范人物关联多个 provider 身份；无稳定身份的演员只写入出演关系。
...
- [ ] 无 ID 的 NFO 演员在后台扫描后出现在详情页，并显示占位头像。
- [ ] 同一规范人物关联 TMDb、IMDb、豆瓣身份时只保留一份人物资源和可复用图片引用。
- [ ] 相同图片内容跨 provider 下载时只保存一份内容哈希资源；不同人物不会仅因姓名相同而自动合并。
- [ ] 明确的跨 provider ID 或管理员确认可以合并人物；未确认的同名人物保持隔离。
- [ ] `people.json` 关系快照升级可读取版本 1，旧人物目录、旧图片索引和 Emby 兼容图片路由继续可用。
- [ ] 演员关系写入、人物资源写入和图片下载彼此解耦；任一资源失败都不丢失演员展示关系。

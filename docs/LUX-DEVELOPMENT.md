...
- 空库初始没有注册任务。创建媒体库时由系统原子注册“全量校验媒体库”和“元数据刮削”两个任务；插件启用后注册插件提供的全局计划任务。所有已注册的 Cron 任务都由管理员在任务页配置执行时间；实时增量扫描由文件系统监听触发，不注册为计划任务。
- 待处理匹配页。
- 元数据编辑与锁定。
- 图片管理。
- 用户与权限。
...
- 季集号、特别篇和缺季目录有测试。
- 逻辑 ID 在重扫后稳定。
- 同名同年份但身份不同的剧集可以共存；重复扫描使用 `identity_key` 恢复既有层级，不因标题唯一性冲突而中止整库校验。
...
期间，其他媒体库的扫描任务保持排队；锁覆盖文件系统发现和条目落库阶段，而不是只覆盖单个
批次。扫描完成后先释放锁，再执行媒体探测、元数据、缩略图和封面等可能耗时的后处理，避免
已完成的媒体库长期占用扫描互斥。该机制是扫描互斥，不引入跨库 worker pool，也不改变任务的
持久化、恢复和取消模型。
...
实施记录（2026-08-14）：扫描锁在文件系统扫描完成后释放；PENDING 任务取消会立即落为
CANCELLED，并清理调和工作项。专项扫描测试、Rust 全量测试和静态检查通过；原生验证架构为
`arm64`。
...
- [x] 仅路径/未知目标触发解析；HTTP(S) 目标保持既有直连合同。
...
使用规范人物目录，provider 身份只作为人物身份索引，不再天然决定人物资源目录。旧
...
IMDb、豆瓣等身份通过 provider-scoped identity 索引关联到规范人物。规范人物目录保存 `person.json`
和人物图片，图片资源采用内容哈希去重；无法确认身份的演员只保存在媒体条目的 `people.json`，不创建
人物目录、不下载头像。旧 provider 目录和关系快照只读兼容，升级不得删除旧文件。

验收：

- [x] 无 ID 的 NFO 演员在后台扫描后出现在详情页，并显示占位头像。
- [x] 同一出演关系携带 TMDb、IMDb、豆瓣身份时只保留一份规范人物资源和可复用图片引用。
- [x] 相同图片内容跨 provider 下载时只保存一份内容哈希资源；不同人物不会仅因姓名相同而自动合并。
- [ ] 明确的跨 provider ID 或管理员确认可以合并人物；未确认的同名人物保持隔离。
- [x] `people.json` 关系快照升级可读取版本 1，旧人物目录、旧图片索引和 Emby 兼容图片路由继续可用。
- [x] 演员关系写入、人物资源写入和图片下载彼此解耦；任一资源失败都不丢失演员展示关系。

验证：人物关系单测、NFO 扫描集成测试、跨 provider 合并/隔离测试、图片内容去重测试、旧布局兼容测试、
详情 API 和 Web 占位图测试。

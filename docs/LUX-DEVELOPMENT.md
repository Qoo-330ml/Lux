...
- ffprobe 默认并发 8，可按媒体库配置 1 至 32；实际运行的单库上限为 16、进程全局上限为 16，
  并根据 CPU、内存和前台 p95 动态降档。4 核 NAS 的默认有效并发目标为 8；ffprobe 只处理本轮
...
ffprobe 使用独立的有界资源配额：默认 8 路，单库有效上限 16，进程全局有效上限 16；配置值
保留 1 至 32 的输入范围，但实际并发受 CPU、内存、前台 p95 和全局 semaphore 限制。后处理
...
- [ ] ffprobe 默认有效并发为 8，硬上限为 16；CPU、内存或前台 p95 恶化时能动态降档，恢复后带冷却地升档。
- [ ] reconciliation 工作项的发现、seen、变化目标登记和完成清理使用有界批量事务，SQLite 不逐文件往返。
- [ ] 4/8/12/16 路 ffprobe 和 60,000 文件首扫/无变化重扫均有可重复基准记录；扫描期间前台 p95 保持小于 1 秒或记录差距。

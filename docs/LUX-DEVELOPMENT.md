...
- 规范化标题完全一致、媒体类型一致、年份相同或相差不超过 1 年，且最佳候选达到高置信度：可自动确认；多个候选按最高分选择，但最高分与次高分差小于 5 分时进入“待确认”（包括完全同分），避免相近候选被静默选错。
- 其他情况：待处理。
...
IMDb、豆瓣等身份通过 provider-scoped identity 索引关联到规范人物。规范人物目录保存 `person.nfo` 和
Emby 风格的 `folder.<ext>` 人物图片；图片字节以 `people/assets/<sha256>.<ext>` 内容哈希资源保存，人物
目录中的 `folder.<ext>` 使用同一配置卷内的硬链接引用该资源，从而同时满足目录自包含和跨 provider 去重。
无法确认身份的演员只保存在媒体条目的 `people.json`，不创建人物目录、不下载头像。旧 provider 目录、
已有共享 assets 和关系快照继续兼容读取，升级不得删除旧文件；新的内容哈希资源仍写入共享 assets 池。

验收：

- [x] 无 ID 的 NFO 演员在后台扫描后出现在详情页，并显示占位头像。
- [x] 同一出演关系携带 TMDb、IMDb、豆瓣身份时只保留一份规范人物资源和可复用图片引用。
- [x] 相同图片内容跨 provider 下载时只保存一份内容哈希资源，并在每个规范人物目录提供
  `folder.<ext>` 硬链接；不同人物不会仅因姓名相同而自动合并。

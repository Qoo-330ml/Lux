...
- ffprobe 默认并发 16，可按媒体库配置 1 至 64；实际运行的单库有效上限为 32、进程全局硬上限为 32，
  并根据 CPU、内存和前台 p95 动态降档。4 核 NAS 的默认有效并发目标为 16；ffprobe 只处理本轮
...
ffprobe 使用独立的有界资源配额：默认 16 路，单库有效上限 32，进程全局硬上限 32；配置值
...
- [ ] ffprobe 默认有效并发为 16，硬上限为 32；CPU、内存或前台 p95 恶化时能动态降档，恢复后带冷却地升档。
- [ ] reconciliation 工作项的发现、seen、变化目标登记和完成清理使用有界批量事务，SQLite 不逐文件往返。
- [ ] 8/16/24/32 路 ffprobe 和 60,000 文件首扫/无变化重扫均有可重复基准记录；扫描期间前台 p95 保持小于 1 秒或记录差距。

...
- 每个媒体库默认实时监听文件系统；新增、修改、重命名和删除事件只触发受影响路径的局部增量扫描。实时监听不是可关闭的媒体库开关；旧版 `realtime_watch_enabled` 字段仅作兼容保留并始终按开启处理。增量扫描、全量校验和元数据任务仍可独立配置。
...
- 扫描计划与元数据计划，明确分开。
- 扫描/任务页。
- 任务与日志页集中查看所有已保存的计划配置，并可维护作用于全局或指定媒体库的增量扫描、全量校验和元数据计划。
- 待处理匹配页。
- 元数据编辑与锁定。
- 图片管理。
- 用户与权限。
...
- 显示当前正在播放会话；卡片包含账户、媒体标题/剧集信息、海报、进度、客户端/设备、来源质量、视频轨和音频轨摘要。
...
- 过滤失败类型。
- 日志脱敏。
- 计划任务区分页查看已保存的计划，并可自由新增、修改、停用增量扫描、全量校验和元数据任务；每项计划可作用于全局或指定媒体库。

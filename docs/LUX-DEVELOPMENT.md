...
- ffprobe 默认并发 128，可按媒体库配置 1 至 256；实际运行的单库有效上限为 256、进程全局硬上限为 256，
  并根据 CPU、内存和前台 p95 动态降档。4 核 NAS 的默认有效并发目标为 64，8 核目标为 128，16 核及以上目标为 256；ffprobe 只处理本轮
...
ffprobe 使用独立的有界资源配额：默认 128 路，单库有效上限 256，进程全局硬上限 256；配置值
保留 1 至 256 的输入范围，但实际并发受 CPU、内存、前台 p95 和全局 semaphore 限制。4 核 NAS
的正常 I/O 并行目标为 64，8 核可达到 128，16 核及以上可达到 256；压力升高时按四分之一或二分之一降档，恢复
后经过冷却期逐步翻倍升档。后处理
...
- [x] ffprobe 默认有效并发为 128，硬上限为 256；CPU、内存或前台 p95 恶化时能动态降档，恢复后带冷却地升档。
- [x] reconciliation 工作项的发现、seen、变化目标登记和完成清理使用有界批量事务，SQLite 不逐文件往返。
- [x] 64/128/192/256 路 ffprobe 和 60,000 文件首扫/无变化重扫均有可重复基准记录；扫描期间前台 p95 保持小于 1 秒或记录差距。

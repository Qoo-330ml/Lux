...
Emby 风格的 `folder.<ext>` 人物图片；同一规范人物的多个 provider 身份只共用该目录中的一份图片。
当目录已有头像时，后续 provider 刮削跳过图片写入；人物 NFO 按字段补充，已有非空字段不被覆盖。无法
确认身份的演员只保存在媒体条目的 `people.json`，不创建人物目录、不下载头像。旧 provider 目录、旧
`people/assets` 文件和关系快照继续兼容读取，升级不得删除旧文件；新图片不得写入 `people/assets`。

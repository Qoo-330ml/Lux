...
`home` 作用域：首个扫描变更立即发送，连续扫描批次在 1 秒窗口内合并，扫描终态立即发送；
普通用户 Web 客户端收到后失效首页、媒体库列表和当前媒体库分页缓存；
断线时继续保留低频轮询兜底。该端点不向 Emby 兼容 API 或未认证请求开放。

实施说明（2026-09-21）：扫描专用首页缓存失效与用户事件均使用 1 秒有界合并窗口；普通
用户操作仍使用同步首页失效，扫描终态（完成、失败或取消）时强制 flush，避免中间批次刷新
风暴同时保留最终结果的即时可见性。
...
- [ ] 首个扫描写入和扫描终态使普通用户首页、媒体库列表立即刷新；连续扫描批次最多延迟
  1 秒合并刷新，事件不携带业务数据。

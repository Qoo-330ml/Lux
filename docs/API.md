...
- `GET|HEAD /Videos/{itemId}/master.m3u8`：读取 `PlaybackInfo` 返回的签名 Emby HLS VOD 清单；媒体总时长已知时，清单立即列出全部 `/Videos/{itemId}/transcoding/{sessionId}/{asset}` 分片并包含 `ENDLIST`。`PlaybackInfo` 和这份完整 VOD 清单只登记并保留会话，不占用 FFmpeg 并发名额，也不替换同一 source 正在播放的旧会话；第一个 `init.mp4` 或 `.m4s` 资源请求才替换旧会话、取得名额并启动 generation 0。首个媒体分片直接决定实际起点；若 init 先到，则 `StartTimeTicks` 只作为 generation 0 的首选起点。媒体总时长未知、无法生成完整 VOD 清单时，`index.m3u8` 请求会作为首个实际资源请求启动 FFmpeg，并回退到其物理清单。尚未生成且远离当前输出窗口的逻辑分片会让同一会话的 FFmpeg 从该分片对应的媒体位置重启；视频转码档位按四秒关键帧节奏输出，复制视频流的档位仍可能按源关键帧落点。后续重启使用独立的内部 generation 清单和 init 文件并保留源时间戳，init 与媒体分片并发请求会跟随当前 generation，不会读到被替换 generation 的 init 或返回 seek superseded；相邻分片请求等待当前 FFmpeg 输出，已被更新 seek 取代的旧媒体分片请求不会反向重启会话。两类资源不要求 Web Cookie，仍绑定用户、条目、媒体源、转码会话和过期签名；也接受 `/emby`、小写 `/videos` 兼容前缀。

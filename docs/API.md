...
- 当客户端没有发送 `StartTimeTicks` 时，若用户存在未标记为已播放的保存进度，Lux 会把该进度作为转码 URL 的兼容提示；客户端显式发送的值（包括 `0`）优先于保存进度。该提示不会把续播点的媒体内容伪装为逻辑第 `0` 段。
...
- `GET|HEAD /Videos/{itemId}/master.m3u8`：读取 `PlaybackInfo` 返回的签名 Emby HLS VOD 清单；清单按完整媒体时长列出全部 `/Videos/{itemId}/transcoding/{sessionId}/{asset}` 分片并包含 `ENDLIST`。尚未生成且远离当前输出窗口的逻辑分片会让同一会话的 FFmpeg 从该分片对应的媒体位置重启；视频转码档位按四秒关键帧节奏输出，复制视频流的档位仍可能按源关键帧落点。每次重启使用独立的内部 generation 清单并保留源时间戳，不会删除客户端正在读取的初始清单，也不会把重启后的中段时间重置为零；相邻分片请求等待当前 FFmpeg 输出，已被更新 seek 取代的旧请求不会反向重启会话。清单中的地址读取签名的 `init.mp4` 或 `.m4s` 片段。两类资源不要求 Web Cookie，仍绑定用户、条目、媒体源、转码会话和过期签名；也接受 `/emby`、小写 `/videos` 兼容前缀。

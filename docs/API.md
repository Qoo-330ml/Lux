...
- `GET|HEAD /Videos/{itemId}/master.m3u8`：读取 `PlaybackInfo` 返回的签名 Emby HLS VOD 清单；媒体总时长已知时，清单立即列出全部 `/Videos/{itemId}/transcoding/{sessionId}/{asset}` 分片并包含 `ENDLIST`。每个逻辑 `segment_N.m4s` 前都有对应的 `EXT-X-MAP` `init_N.mp4`，客户端不能把不同逻辑分片的 init 混用。`PlaybackInfo` 和这份完整 VOD 清单只登记并保留会话，不占用 FFmpeg 并发名额，也不替换同一 source 正在播放的旧会话；第一个 `init_N.mp4` 或 `.m4s` 资源请求才替换旧会话、取得名额并启动 generation 0。首个媒体分片直接决定实际起点；若 init 先到，则 `StartTimeTicks` 只作为 generation 0 的首选起点。媒体总时长未知、无法生成完整 VOD 清单时，`index.m3u8` 请求会作为首个实际资源请求启动 FFmpeg，并回退到其物理清单。尚未生成且远离当前输出窗口的逻辑分片会让同一会话的 FFmpeg 从该分片对应的媒体位置重启；视频转码档位按四秒关键帧节奏输出，复制视频流的档位仍可能按源关键帧落点。不同 generation 的 init 和 segment 使用隔离的内部文件名；已绑定的逻辑分片继续读取原 generation，不能跟随后来 seek 的 generation，未完成且被更新 seek 取代的旧请求返回 superseded。两类资源不要求 Web Cookie，仍绑定用户、条目、媒体源、转码会话和过期签名；也接受 `/emby`、小写 `/videos` 兼容前缀。

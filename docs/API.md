...
- `PATCH /api/v1/admin/libraries/{libraryId}`：运行时更新扫描/探测并发、`scraperId` 和媒体库策略覆盖。实时增量扫描由文件系统监听触发，不接受计划配置。历史 `realtimeWatchEnabled`、`incrementalSchedule`、`reconciliationSchedule` 和 `metadataSchedule` 字段会被兼容接受但忽略，响应中的对应计划字段始终为 `null`。字段均可省略；`scraperId` 和 `mediaStrategy` 使用 `null` 清空，非空字符串最长 128 个字符；并发范围为 1-64。`mediaStrategy` 的结构与全局策略相同，未设置时返回 `null` 并继承全局默认。例如 `{ "scraperId": "tmdb", "reconciliationSchedule": "0 3 * * *", "metadataSchedule": "interval:5m" }` 中的两个计划字段仅用于兼容旧客户端。具体执行时间由宿主机 crontab 管理；刮削器必须已安装且配置完成。
...
- `GET /api/v1/admin/scheduled-tasks?page=1&pageSize=100`：分页查看所有已注册的任务，包含 `ownerType`、媒体库名称、`taskType`、`name`、`description`、`sourceType`、可空 `pluginId`、历史兼容字段 `schedule`（始终为 `null`）、启用状态、资源限制和更新时间；结果也包含已停用的注册项。
- `PUT /api/v1/admin/scheduled-tasks`：只修改已注册任务是否允许入队。请求体为 `{ "ownerType": "LIBRARY", "ownerId": "...", "taskType": "RECONCILIATION_SCAN|METADATA_PARSE", "isEnabled": true }`；历史客户端发送的 `schedule` 字段会被忽略。实时增量扫描（`INCREMENTAL_SCAN`）由文件系统事件触发，不属于此接口管理范围。不存在的注册项返回 404，不会因为管理请求凭空创建任务。写操作需要管理员 Web session 和 CSRF。具体执行时间由宿主机 crontab 管理。
- `POST /api/v1/cron/tasks/{ownerType}/{ownerId}/{taskType}`：宿主机 crontab 调用的受保护入队接口，不需要 Web session 或 CSRF；请求头必须为 `Authorization: Bearer <LUX_CRON_TOKEN>`。允许的任务为媒体库的 `RECONCILIATION_SCAN`、`METADATA_PARSE`，以及全局 `STRM_MEDIA_INFO`。任务必须已注册且启用；成功返回 202 并加入现有持久化后台队列，任务已运行时返回 409。令牌通过环境变量 `LUX_CRON_TOKEN` 或配置目录中的 `cron-token` 文件注入，不能写入日志、数据库或响应。Lux 不解析 cron 表达式，也不运行内部计划轮询器。

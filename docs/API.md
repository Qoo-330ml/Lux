...
- 当客户端没有发送 `StartTimeTicks` 时，若用户存在未标记为已播放的保存进度，Lux 会将该进度用于服务端 HLS 的起始 seek；客户端显式发送的值（包括 `0`）优先于保存进度。
  Emby HLS 清单本身按完整 `RunTimeTicks` 返回 VOD 时间轴；`RunTimeTicks` 同时保留在响应中供客户端协议兼容和进度回调使用。
...
- `GET|HEAD /Videos/{itemId}/master.m3u8`：读取 `PlaybackInfo` 返回的签名 Emby HLS VOD 清单；清单按完整媒体时长列出全部 `/Videos/{itemId}/transcoding/{sessionId}/{asset}` 分片并包含 `ENDLIST`，尚未生成的分片请求会等待对应 FFmpeg 输出。清单中的地址读取签名的 `init.mp4` 或 `.m4s` 片段。两类资源不要求 Web Cookie，仍绑定用户、条目、媒体源、转码会话和过期签名；也接受 `/emby`、小写 `/videos` 兼容前缀。

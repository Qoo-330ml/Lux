...
- `GET|HEAD /Videos/{itemId}/master.m3u8`：读取 `PlaybackInfo` 返回的签名 Emby HLS 清单；清单声明 VOD playlist 类型但在转码完成前仍动态追加片段，不提前写入 `ENDLIST`；清单中的 `/Videos/{itemId}/transcoding/{sessionId}/{asset}` 地址读取签名的 `init.mp4` 或 `.m4s` 片段。两类资源不要求 Web Cookie，仍绑定用户、条目、媒体源、转码会话和过期签名；也接受 `/emby`、小写 `/videos` 兼容前缀。

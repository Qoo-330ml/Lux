...
- 新增可选 Chrome MV3 扩展桥接：为没有 CORS 的远程 Matroska/WebM 提供跨域字幕 Range 读取，并将 SRT/ASS/SSA cue 返回 Lux 覆盖层；扩展不接管原生音视频，也不向 Lux 上传媒体字节。
...
- 对 Chrome 不支持的远程 AC-3/E-AC-3 音轨显示明确提示；字幕扩展只解析字幕，不承担音频解码。
...
- 远程 `.strm` 默认仍使用原生播放并优先利用浏览器实际暴露的 `TextTrack`；仅手动选择支持的远程文本轨时启用可选 Chrome 扩展字幕旁路和 Lux 覆盖层，不接管音视频、不重建或停止播放会话，也不切换服务端 HLS。本地字幕处理保持原有边界。
- 显式远程字幕模式仍依赖扩展权限、远端 Range、索引和字幕解封装支持；完整 Cues 跳转及跨浏览器阶段门尚待完成，且不再依赖客户端音视频 MSE codec 组合。

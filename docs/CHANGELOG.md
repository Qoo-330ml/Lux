...
- 修复 Emby 媒体详情 `CanDelete` 固定为 `false` 且缺少 `DELETE /Items/{id}` 兼容路由的问题；现在服务器管理权限用户可通过 Emby token 删除媒体源，普通用户仍被拒绝。
...
- 修复 Emby 客户端播放 URL 型 `.strm` 时 `DirectStreamUrl` 被 Lux 签名入口覆盖的问题；HTTP URL 和本地路径型目标现在保留原始播放目标。
...
- 调整 `.strm` 的 Emby `PlaybackInfo` 兼容策略：URL/路径型目标保留原始 `DirectStreamUrl` 并关闭 `AddApiKeyToDirectStreamUrl`，协议解析器目标继续使用 Lux 受保护入口。

...
- 新增可选 Chrome MV3 全媒体桥接：为没有 CORS 的远程 HTTP(S) Matroska/WebM 建立版本化媒体会话和有界 Range 读取，接入现有 Matroska Worker/MSE 输出音视频；扩展不向 Lux 上传媒体字节。
- 增加实验性的远程 AC-3/E-AC-3 Worker 窄解码与 Web Audio PCM 输出，并复用 HEVC→H.264 软件回退；能力仍需真实 Chrome 设备完成音画同步、seek、暂停和资源上限验收，DTS/TrueHD 等不在支持范围。
- 新增可选 Chrome MV3 字幕桥接：为没有 CORS 的远程 Matroska/WebM 提供跨域字幕 Range 读取，并将 SRT/ASS/SSA cue 返回 Lux 覆盖层；字幕旁路不修改原生音视频请求。
...
- 恢复远程 HTTP(S) `.strm` 的 Matroska/WebM 内嵌文本字幕手动选择：浏览器未暴露原生轨道时，选择 SRT/ASS/SSA 后才通过可选 Chrome MV3 扩展使用当前播放会话签名入口读取字幕，不接管原生音视频、不预先抽取、不落盘或生成外挂文件。
...
- 修复远程 Matroska 显式字幕读取切换客户端音视频引擎、导致原生视频受 CORS、Range、索引或 MSE 条件影响的问题；现在保持原生 `<video>` Direct Play，字幕旁路失败只清除字幕状态。
...
- 修复选择远程内嵌字幕时因切换客户端音视频引擎而破坏原生 HEVC/E-AC-3 等播放的问题；现在字幕仅通过签名 Range 旁路读取，保持原生 `<video>` 音视频、seek 和播放会话。
- 修复 Chrome 扩展发现 Matroska `TrackUID` 后反复重启字幕桥、导致已选字幕被清空的问题；字幕桥现在以 API `streamIndex` 作为稳定请求身份。移除扩展对原生媒体响应头的动态修改，避免影响原生音视频。
- 修复 Chrome 扩展媒体能力探测把 fMP4 MSE 支持误当作 Matroska 原生支持的问题；现在仅在原生 Matroska 无法播放，或需要软件音频时选择全媒体扩展路径。
- 修复扩展媒体 Range 的响应长度、Content-Range、总长度和 ETag 稳定性校验，以及会话停止时未及时中止读取的问题；失败保留具体诊断，不静默破坏原生播放。
- 对未启用或不可用全媒体扩展的 Chrome 不支持的远程 AC-3/E-AC-3 音轨显示明确提示；全媒体扩展可选用实验性 Worker/Web Audio 音频路径，字幕旁路仍不承担音频解码。
...
- 远程 `.strm` 默认仍使用原生播放并优先利用浏览器实际暴露的 `TextTrack`；原生 Matroska 无法处理当前音视频组合时，才按能力选择可选 Chrome 全媒体扩展，字幕旁路也可独立启用，不修改原生媒体请求、不重建或停止播放会话，也不切换服务端 HLS。本地字幕处理保持原有边界。
- 全媒体扩展和显式远程字幕模式仍依赖扩展权限、签名入口、远端 Range、索引和有界解封装支持；完整 Cues 跳转、软件解码资源上限及跨浏览器阶段门尚待完成。

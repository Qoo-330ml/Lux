...
- 恢复远程 HTTP(S) `.strm` 的 Matroska/WebM 内嵌文本字幕手动选择：浏览器未暴露原生轨道时，选择 SRT/ASS/SSA 后才通过当前播放会话签名的同源 Range Relay 启动字幕旁路读取器，不接管原生音视频、不预先抽取、不落盘或生成外挂文件。
...
- 远程 `.strm` 默认仍使用原生播放并优先利用浏览器实际暴露的 `TextTrack`；仅手动选择支持的远程文本轨时启用字幕旁路读取器、同源 Range Relay 和 Lux 覆盖层，不接管音视频、不重建或停止播放会话，也不切换服务端 HLS。本地字幕处理保持原有边界。
- 显式远程字幕模式仍依赖 Range、索引和字幕解封装支持；完整 Cues 跳转及跨浏览器阶段门尚待完成，且不再依赖客户端音视频 MSE codec 组合。

...
为 `org.lux.tmdb` 提供可持久化的首选语言、语言回退和替代 API 地址设置。TMDb 插件使用首选语言请求电影、剧集、季度和单集元数据；开启回退后，按管理员选择的语言顺序逐字段补全空值。替代 API 地址开关默认关闭，开启后可选择官方地址、`https://api.tmdb.org` 或填写自定义 HTTP(S) 基础地址。
...
5. 在 Web 插件配置对话框中加入替代 API 地址开关、预设地址下拉和自定义地址输入，并让独立插件使用该配置。

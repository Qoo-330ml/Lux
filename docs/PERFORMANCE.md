...
| 2026-08-17 | 9ccdb79 | 飞牛 x86_64（10.0.0.248） | PostgreSQL 17；833,496 条 `media_items`；2 个媒体库 | 登录后 `curl` 请求 `GET /api/v1/home`；冷请求 1 次，缓存命中请求 20 次 | 3.8 ms | 5.3 ms | 0% | - | 冷请求 3,361 ms；命中缓存后 p50 3.8 ms、p95 5.3 ms；缓存过期时先返回旧快照并后台刷新；未执行扫描期间基准，不能据此宣称满足完整 LUX-082 目标 |
| 2026-08-17 | b48dcfa | 飞牛 x86_64/amd64（10.0.0.248） | PostgreSQL 17；833,496 条 `media_items`；2 个媒体库；扫描同时运行 | 容器重启后登录，直连 `GET /api/v1/home`；首次请求 1 次，随后缓存请求 5 次 | 11,487 ms | 13,239 ms | 0% | 11,522 ms（首次） | 首次请求和 5 次缓存请求均 HTTP 200；扫描持续产生缓存失效时前台不再循环等待；PostgreSQL 未重启；公网反向代理复核 HTTP 200/10,371 ms；仍受扫描和数据库写入负载影响，不能据此宣称满足完整 LUX-082 目标 |
| 2026-08-17 | 工作树（home-cache-stale-v5） | 飞牛 x86_64/amd64（10.0.0.248） | PostgreSQL 17；833,496 条 `media_items`；2 个媒体库；`RECONCILE_LIBRARY` 与 STRM 探测仍在运行 | 已登录 Chrome 连续导航到首页 8 次，等待“我的媒体库”可见 | 325 ms | 355 ms | 0% | 355 ms | 新镜像重启后首页完整文档加载单次 218–236 ms；扫描期间仍保持亚秒级；stale-while-revalidate 命中最近完整用户快照，后台刷新不阻塞前台；与上一条直连 API 基线的测量方式不同，作为部署验证记录，不直接替代同命令前后对比 |

## 媒体库首屏加载

| 日期 | 版本/索引 | 硬件/架构 | 数据集 | 场景 | 冷请求 | 缓存命中 | 错误率 | 备注 |
|---|---|---|---|---|---:|---:|---:|---|
| 2026-08-17 | `library-cache-v1`，无类型索引 | 飞牛 x86_64/amd64（10.0.0.248） | PostgreSQL 17；电视剧库 793,293 条记录，其中 25,306 个可见剧集 | 重启 Lux 清空进程缓存后，登录 Chrome 首次打开电视剧库 | 6.3 s | 0.62–0.64 s | 0% | 后台数据库负载同时存在；作为同机优化前基线 |
| 2026-08-17 | `library-cache-v5`，`idx_media_items_library_type_visible` | 飞牛 x86_64/amd64（10.0.0.248） | 同上 | 重启 Lux 清空进程缓存后，登录 Chrome 连续打开电视剧库 3 次 | 2.1 s | 0.61–0.89 s | 0% | 冷请求约减少 66%；缓存命中保持亚秒级；数据库 schema 23、容器 healthy；当前无扫描任务，不能外推扫描高峰性能 |

...
- URL 型目标继续保留原始外部 URL 和 HTTP 协议，`DirectStreamUrl` 直接返回原始目标；受保护的 Lux 视频入口仅对兼容客户端返回原始目标的 307，不在服务端请求或解析上游。这样第三方服务可以收到实际播放器的 User-Agent，不绑定 `/strm/api/302` 等具体路径，也不代理媒体字节。

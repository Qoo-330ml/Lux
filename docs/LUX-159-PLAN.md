...
- URL 型目标继续保留原始外部 URL 和 HTTP 协议，但 `DirectStreamUrl` 指向受保护的 Lux 视频入口；该入口由 Lux 服务端对任意 HTTP(S) 目标执行有限重定向解析，200/206 媒体响应回退为原始目标，不绑定 `/strm/api/302` 等具体路径，也不代理媒体字节。

...
- Lux 核心负责任务、媒体库选择、媒体路径和 STRM 探测目标长度校验、并发控制、插件监督、结果落库和旁车文件写入。
- 插件只接收单个已通过宿主校验的 STRM 探测目标，不能访问 Lux SQLite、媒体根目录或内部任务对象。
...
一旦 Lux 主动访问 `.strm` 探测目标，原本“只把目标交给客户端播放”的边界就改变为服务器端
出站请求。目标本身不解析、不分类，由部署者通过进程权限和网络出口策略控制可访问范围。

当前实现：

- STRM 第一条非空内容按原始字符串传递，不解析 URL，不区分 HTTP/HTTPS、file、data、ftp、Unix socket、域名、IP 或路径。
- 宿主和媒体探测插件只执行非空和长度校验；策略失败只产生安全摘要，不返回完整探测目标。
- 不在日志、错误消息、任务事件或插件 RPC 中暴露完整探测目标。
- 不把探测目标复制到旁车或返回给 Web 客户端。

当前仍有一个生产化风险需要后续处理：ffprobe 内部重定向的逐跳策略。由于目标按原始字符串
交给 `ffprobe`，部署者应通过进程权限和网络出口策略限制 Lux 进程能够访问的地址；应用层不
要求管理员额外指定 IP 或网段。
...
插件启用后接入 Lux 计划调度器，复用同一 STRM 应用服务。计划使用插件配置中的五段式 cron（例如
`0 3 * * *`），首次执行在后台完成；禁用插件会停用注册任务，服务重启后从持久化任务配置恢复。管理员
仍可通过现有手动触发入口立即执行一次。
...
1. 是否为带 query token 的目标增加受控凭据策略；当前实现不会把探测目标写入日志或旁车。
2. 是否在调度器完成后接入计划任务；当前只提供管理员手动触发。
3. 是否增加 ffprobe 重定向逐跳校验；当前实现已明确将其作为生产化前置风险。

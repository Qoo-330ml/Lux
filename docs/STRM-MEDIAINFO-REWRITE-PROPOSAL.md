...
> 状态：已选择插件路线；核心实现、管理 Web 页面和宿主机 crontab 入队已落地
>
> 日期：2026-08-05
>
...
| 右键/计划任务 | 管理 API 触发；宿主机 crontab 调用 Lux cron 入队接口 | Lux 不运行内部计划调度器 |
...
插件启用后登记一个全局 `STRM_MEDIA_INFO` 任务，复用同一 STRM 应用服务。宿主机 crontab 调用
Lux cron 入队接口，首次执行和后续执行都在持久化后台完成；禁用插件会停用注册任务。管理员仍可
通过现有手动触发入口立即执行一次。
...
3. 是否接入计划任务；已确定由宿主机 crontab 调用 Lux cron 入队接口，Lux 不提供内部 cron 解析器。

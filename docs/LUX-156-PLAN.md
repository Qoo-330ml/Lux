...
`<config>/logs/lux.YYYY-MM-DD.log`，并允许管理员从控制台按日期导出原始日文件；选择多天时
导出 ZIP，便于分析扫描、图片和请求错误。

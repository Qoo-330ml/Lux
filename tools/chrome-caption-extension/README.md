...
- 扩展在当前 Lux 页面来源下为远程媒体响应补充 CORS 响应头，使 Lux 已有客户端解封装/WASM fallback 可以直接读取远端字节；字幕读取不携带页面 Cookie，Lux 服务器只参与一次授权/307 解析，媒体内容从 307 之后的远端地址到浏览器。
...
扩展需要远程主机权限，因为远程 STRM 的 CDN 通常没有 CORS；权限只用于当前 Lux 页面发起的媒体请求。发布包由 `scripts/build-chrome-caption-extension.sh` 生成。

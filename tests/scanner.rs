...
async fn media_catalog_migration_creates_expected_tables() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let database = Database::connect(&config).await?;
    assert_eq!(database.schema_version().await?, 6);
    let tables: Vec<String> = sqlx::query_scalar(
        "SELECT name FROM sqlite_master
         WHERE type = 'table' AND name IN ('filesystem_entries', 'media_items', 'media_sources')
         ORDER BY name",
    )
    ...
    assert_eq!(
        tables,
        ["filesystem_entries", "media_items", "media_sources"]
    );
    Ok(())
}

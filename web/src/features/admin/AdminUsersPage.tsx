...
import { Check, KeyRound, Plus, ShieldCheck, UserRound, X } from "lucide-react";
...
function AdminUserState({ label, error = false }: { label: string; error?: boolean }) { return <section className="lux-admin-page-state" role={error ? "alert" : "status"}><span className="lux-eyebrow">LUX ADMIN</span><h1>{error ? "用户数据加载失败" : "正在加载用户"}</h1><p>{label}</p></section>; }

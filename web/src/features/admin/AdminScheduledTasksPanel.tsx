...
export function AdminScheduledTasksPanel() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setSavedMessage("");
    if (!libraryId) {
      setSavedMessage("请先选择媒体库");
      return;
    }
    update.mutate({ ownerType: "LIBRARY", ownerId: libraryId, taskType, schedule: schedule.trim() || null, isEnabled: isEnabled && Boolean(schedule.trim()) });
  };
  ...
  return (
    <section className="lux-admin-panel lux-admin-operations-section" aria-labelledby="scheduled-tasks-heading">
      ...
      <p className="lux-admin-muted">集中查看媒体库页设置的计划，并在这里快速调整增量扫描、全量校验和元数据任务。</p>
      <form className="lux-admin-form lux-admin-schedule-form" onSubmit={submit}>
        <label htmlFor="schedule-library">媒体库<select id="schedule-library" name="schedule-library" value={libraryId} onChange={(event) => setLibraryId(event.target.value)}>
          <option value="">选择媒体库</option>
          {libraryItems.filter((library) => library.isEnabled).map((library) => <option key={library.id} value={library.id}>{library.name}</option>)}
        </select></label>
...
function ScheduleRow({ task, onEdit, onDisable }: { task: AdminScheduledTask; onEdit: () => void; onDisable: () => void }) {
  ...
  const canEdit = task.ownerType === "LIBRARY" && SCHEDULE_TASKS.some((item) => item.value === task.taskType);

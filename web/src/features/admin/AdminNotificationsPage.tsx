...
export function AdminNotificationsPage() {
  ...
  if (providers.isPending || destinations.isPending || deliveries.isPending) return <AdminNotificationsState label="正在读取通知配置…" />;
  if (providers.error || destinations.error || deliveries.error) return <AdminNotificationsState label={(providers.error || destinations.error || deliveries.error)?.message || "通知数据加载失败"} error />;
  ...
  return <main className="lux-admin-page lux-notifications-page">
    ...
    <DeliveryList deliveries={deliveryItems} onRetry={() => void invalidateNotificationQueries(queryClient)} />
...
function DestinationRow({ destination, providers, onSecret, onChanged }: { destination: AdminWebhookDestination; providers: AdminPlugin[]; onSecret: (secret: string) => void; onChanged: () => void }) {
  const [editing, setEditing] = useState(false); const [name, setName] = useState(destination.name); const [url, setUrl] = useState(destination.url); const [enabled, setEnabled] = useState(destination.enabled); const [allowPrivateNetwork, setAllowPrivateNetwork] = useState(destination.allowPrivateNetwork); const [eventTypes, setEventTypes] = useState(destination.eventTypes); const [providerPluginId, setProviderPluginId] = useState(destination.providerPluginId); const [providerConfig, setProviderConfig] = useState(destination.providerConfig);
  const provider = providers.find((item) => item.id === destination.providerPluginId); const editingProvider = providers.find((item) => item.id === providerPluginId);
  const update = useMutation({ mutationFn: () => api.updateAdminWebhookDestination(destination.id, { name: name.trim(), url: url.trim(), enabled, allowPrivateNetwork, eventTypes, providerPluginId, providerConfig }), onSuccess: () => { setEditing(false); onChanged(); } });

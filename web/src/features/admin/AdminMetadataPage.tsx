...
export function AdminMetadataPage() {
  ...
  return <div className="lux-admin-page"><header className="lux-admin-page-heading"><div><span className="lux-eyebrow">METADATA REVIEW</span><h1>元数据纠错</h1><p>查看低置信度匹配，确认候选并写回 NFO 与图片。</p></div><button className="lux-button lux-button-secondary" type="button" onClick={() => void pending.refetch()}><RefreshCw size={16} /> 刷新</button></header><section className="lux-admin-panel"><div className="lux-admin-panel-heading"><div><span className="lux-eyebrow">PENDING MATCHES</span><h2>待处理候选</h2></div><span className="lux-status-pill">{pending.data.total ?? items.length} 个</span></div>{items.length === 0 ? <div className="lux-admin-empty"><Check size={25} /><h2>没有待处理匹配</h2><p>当前所有条目的元数据都已确认。</p></div> : <div className="lux-admin-metadata-list">{items.map((item) => <MetadataCandidateRow key={item.id} item={item} onSelected={() => { void queryClient.invalidateQueries({ queryKey: queryKeys.adminPendingMetadata }); }} />)}</div>}</section></div>;
}
...
function AdminMetadataState({ label, error = false }: { label: string; error?: boolean }) { return <section className="lux-admin-page-state" role={error ? "alert" : "status"}><span className="lux-eyebrow">LUX ADMIN</span><h1>{error ? "元数据加载失败" : "正在加载元数据"}</h1><p>{label}</p></section>; }

...
import { prefetchLibraryPage } from "./prefetchLibrary";
  ...
  return (
    <section className="lux-page lux-page-narrow">
      ...
      <div className="lux-library-grid">
        {items.length ? items.map((library) => <LibraryCard key={library.id} library={library} onPrefetch={() => void prefetchLibraryPage(queryClient, library)} />) : <div className="lux-empty-card"><span>当前账号没有可访问的媒体库。</span><Link to="/">返回首页</Link></div>}
      </div>
    </section>
  );
}

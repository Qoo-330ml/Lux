...
test("management navigation exposes the documented sections without a separate metadata inbox", () => {
  assert.deepEqual(
    ADMIN_NAV_ITEMS.map((item) => item.id),
    ["dashboard", "libraries", "users", "jobs", "settings"],
  );
  assert.deepEqual(
    ADMIN_NAV_ITEMS.map((item) => item.label),
    ["仪表盘", "媒体库与计划", "用户与权限", "任务与日志", "服务端设置"],
  );
});

...
const dashboard: AdminDashboard = {
  server: { name: "客厅 Lux", version: "0.1.16", commit: "abc1234", schemaVersion: 37 },
...
describe("AdminDashboardPage", () => {
  ...
  it("renders server identity, rich playback cards, and account activity", async () => {
    ...
    expect(container.textContent).toContain("v0.1.16");
    ...
    expect(overview?.querySelector('[data-overview-value="版本"] strong')?.textContent).toBe("v0.1.16");
    expect(overview?.querySelector('[data-overview-value="运行时长"] strong')?.textContent).toBe("1天 1时 1分 1秒");
    expect(overview?.querySelector('[data-overview-value="版本"]')?.textContent).toBe("版本：v0.1.16");

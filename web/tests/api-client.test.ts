...
describe("LuxApiClient", () => {
  ...
  it("exposes the paginated plugin catalog and installs a store plugin with CSRF", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      ...
      if (path === "/api/v1/admin/plugins/tmdb/upgrade") {
        expect(init?.method).toBe("POST");
        expect((init?.headers as Headers).get("X-CSRF-Token")).toBe("csrf-token");
        return new Response(JSON.stringify({ plugin: { id: "tmdb", installed: true, version: "1.1.0" } }), { status: 200 });
      }
    ...
    await expect(client.upgradeAdminPlugin("tmdb")).resolves.toEqual({ plugin: { id: "tmdb", installed: true, version: "1.1.0" } });

...
test("LuxPlayer reserves dynamic viewport and safe-area space for mobile controls", () => {
  ...
  assert.match(styles, /\.lux-player-controls-wrap \{[^}]*bottom: calc\(24px \+ env\(safe-area-inset-bottom\)\)/);
  assert.match(styles, /\.lux-player-settings-popover \{[^}]*right: calc\(4% \+ env\(safe-area-inset-right\)\)/);
});

...
mod tests {
    ...
    fn recommendation_stats_cleanup_uses_an_indexable_exists_lookup() {
        assert!(RECOMMENDATION_STATS_CLEANUP_QUERY.contains("WHERE NOT EXISTS"));
        assert!(
            RECOMMENDATION_STATS_CLEANUP_QUERY
                .contains("media_items.id = recommendation_item_stats.item_id")
        );
